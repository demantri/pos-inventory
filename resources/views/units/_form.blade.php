        ...
        <div>
            ...
            <input type="text" id="symbol" name="symbol"
                   ...
                   class="w-full px-3.5 py-2.5 border rounded-lg text-sm font-mono
                          focus:outline-none focus:ring-2 focus:ring-blue-500
                          {{ $errors->has('symbol') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">
    ...
    <div class="flex items-center gap-3">
        <button type="button" id="toggle-active" onclick="toggleActive()"
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors
                       ...
                       {{ old('is_active', $unit->is_active ?? true) ? 'bg-blue-600' : 'bg-gray-200' }}">
            <span id="toggle-knob"
                  class="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform
                         {{ old('is_active', $unit->is_active ?? true) ? 'translate-x-6' : 'translate-x-1' }}">
            </span>
        </button>
        <input type="hidden" name="is_active" id="is_active"
               value="{{ old('is_active', $unit->is_active ?? true) ? '1' : '0' }}">
        <span class="text-sm text-gray-700" id="toggle-label">
            {{ old('is_active', $unit->is_active ?? true) ? 'Aktif' : 'Nonaktif' }}
        </span>
    </div>
</div>
