...
<div class="max-w-lg">
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-semibold text-gray-800">Edit: {{ $unit->name }}</h3>
            <p class="text-sm text-gray-500 mt-0.5">Perbarui informasi satuan</p>
        </div>
        <form method="POST" action="{{ route('units.update', $unit) }}" class="px-6 py-6">
            @csrf @method('PUT')
            @include('units._form')
